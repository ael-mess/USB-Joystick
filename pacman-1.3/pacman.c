...
****************************************************************/
void main_pac_man(int argc, char *argv[100]) {
...
void GetInput() {
    ...
    struct pollfd pwt = { STDIN_FILENO, POLLIN|POLLPRI, 0};
    ...
    case 9:                            //Pause game
        PauseGame();
        if(poll(&pwt, 1, 200)) scanf("%d",&ch);  //Update buffered input
        break;
